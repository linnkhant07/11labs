...
import { NarratorChat, type NarratorChatHandle } from "../components/narrator-chat";
...
function StoryPage() {
  ...
  // If the URL explicitly names a narrator, treat it as an override and sync
  // the context so subsequent pages and a refresh agree.
  useEffect(() => {
  ...
  const demoStory: Story | null = isDemo
    ? {
        ...
        narrator: { type: "animal", character: "mouse", voice_id: "" },
      }
    : null;
  ...
  const [, setHasFinishedReading] = useState(false);
  const [nudgeText, setNudgeText] = useState<string | null>(null);
  const [, setChatActive] = useState(false);
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const inactivityTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const nudgeCount = useRef(0);
  const chatRef = useRef<NarratorChatHandle | null>(null);
      ...
      try {
        ...
        if (cached.ok) {
          ...
          if (cachedVoice && cachedVoice !== voiceId) {
            console.log(`[story] Cached story voice mismatch (selected: ${narratorId}) — audio will regenerate on-demand`);
        ...
        const res = await fetch("/api/generate", {
          ...
          body: JSON.stringify({ topic, narrator: narratorId, voiceId }),
  ...
  const playNarration = useCallback(async (page: Page) => {
    ...
    try {
      const res = await fetch("/api/speak", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: page.narration, voiceId }),
      });
  ...
  const resetInactivityTimer = useCallback(() => {
    ...
    inactivityTimer.current = setTimeout(() => {
      ...
      fetch("/api/speak", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: prompt, voiceId }),
      })
        .then((res) => res.blob())
        .then((blob) => {
          const url = URL.createObjectURL(blob);
    ...
    return (
      <main className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-[#fcebdc] to-white px-6">
        <div className="text-center space-y-6">
          ...
          <p className="font-grandstander text-[18px] text-[#585858]">
            About: {topic}
          </p>
        </div>
      </main>
  ...
  if (error || !story || !currentPage) {
    return (
      <main className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-[#fcebdc] to-white px-6">
        <div className="text-center space-y-6">
          <p className="font-grandstander text-[20px] text-red-600">
            {error ?? "Something went wrong"}
          </p>
          <a
            href="/"
            className="inline-block rounded-full bg-[#f09237] px-8 py-3 font-grandstander text-[18px] font-medium text-white shadow-[2px_4px_0px_0px_#db6c00] transition-transform hover:scale-[1.03]"
          >
            Try Again
          </a>
        </div>
  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-[#fcebdc] to-white">
      ...
      <DecorLeaf
        ...
        size={78}
        rotate={18.88}
        className="left-[2%] top-[48%] hidden md:block"
      />
      <DecorLeaf
        src="/figma/landing/leaf-1.svg"
        size={91}
        rotate={55.56}
        className="right-[3%] top-[5%] hidden md:block"
      />
      <DecorLeaf
        src="/figma/landing/leaf-2.svg"
        size={63}
        rotate={-72.55}
        className="right-[8%] top-[12%] hidden md:block"
      />
      ...
      <div className="relative mx-auto flex w-full max-w-[1280px] flex-col gap-10 px-6 py-10 md:px-16 md:py-20 md:gap-14 lg:px-24">
        ...
        <header className="flex w-full flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-6 md:gap-12">
            ...
            <div className="flex flex-col gap-2 md:gap-3">
              <h1 className="font-chelsea text-[28px] leading-tight text-black md:text-[44px]">
                {story.title}
              </h1>
              <p
        ...
        <section className="flex w-full flex-col items-center gap-8 rounded-[24px] bg-white p-6 shadow-[2px_2px_5px_rgba(0,0,0,0.1)] md:gap-12 md:px-10 md:py-[60px]">
          ...
          <div className="flex w-full flex-col items-center gap-6 md:gap-8">
            <div className="flex items-center justify-center gap-8 md:gap-10">
              <button
                ...
                className="transition-transform hover:scale-110 disabled:opacity-30 disabled:hover:scale-100"
              >
                <ArrowLeftSmall />
              </button>
              ...
              <button
                onClick={handleNext}
                disabled={pageIndex === totalPages - 1 || !!needsChoice}
            ...
            {/* Progress bar — segments equal totalPages */}
            <div className="flex w-full max-w-full gap-3">
              {Array.from({ length: totalPages }).map((_, i) => (
                <div
                  ...
                  style={{
                    backgroundColor: i <= pageIndex ? "#f09237" : "#eaeaea",
                  }}
                />
              ))}
            </div>
            ...
            <div className="relative aspect-[1112/360] w-full">
              {currentPage.image_url ? (
                ...
                <div className={`size-full bg-gradient-to-br ${getGradient(pageIndex)} flex items-center justify-center rounded-[24px]`}>
                  <p className="px-8 text-center text-sm font-medium italic text-white/70">
                    {currentPage.image_prompt}
                  </p>
                </div>
              )}

              {/* Magnifier affordance icon (top-left) */}
          ...
          {/* Narration text */}
          <p
            className="w-full text-[18px] leading-[1.7] text-black md:text-[24px] md:leading-[2]"
            style={{ fontFamily: "var(--font-abeezee), sans-serif" }}
          >
          ...
          {nudgeText && (
            <div
              className="w-full cursor-pointer rounded-2xl border-2 border-[#fee8d3] bg-[#fef5ea] p-4 text-center transition-all hover:bg-[#fee8d3]"
              onClick={() => setNudgeText(null)}
            >
              <p className="font-grandstander text-[16px] font-medium text-[#f09237]">
                {narrator.short}: &ldquo;{nudgeText}&rdquo;
              </p>
          ...
          <div className="relative flex w-full items-center justify-between">
            <button
              ...
              disabled={pageIndex === 0}
              aria-label="Previous page"
              className="flex size-16 items-center justify-center rounded-full border border-[#eaeaea] bg-[#eaeaea] shadow-[2px_2px_5px_rgba(0,0,0,0.1)] transition-transform hover:scale-105 disabled:opacity-40 disabled:hover:scale-100 md:size-20"
            >
              <ArrowLeftLarge />
            </button>
            ...
            <button
              ...
              className="flex size-16 items-center justify-center rounded-full bg-[#f09237] shadow-[2px_2px_10px_rgba(0,0,0,0.1)] transition-transform hover:scale-105 disabled:opacity-60 md:size-20"
            >
              {loading ? (
            ...
            <button
              onClick={isLastPage ? handleRestart : handleNext}
              disabled={!!needsChoice}
              aria-label={isLastPage ? "Restart" : "Next page"}
              className="flex size-16 items-center justify-center rounded-full border border-[#fee8d3] bg-[#fee8d3] shadow-[2px_2px_5px_rgba(0,0,0,0.1)] transition-transform hover:scale-105 disabled:opacity-40 disabled:hover:scale-100 md:size-20"
            >
              <ArrowRightLarge />
            </button>
          </div>
          ...
          {needsChoice && currentPage.choice && (
            <div className="w-full space-y-4 rounded-2xl border-2 border-[#fee8d3] bg-[#fef9f3] p-6">
              <p className="text-center font-grandstander text-[18px] font-medium text-[#f29337] md:text-[22px]">
                {currentPage.choice.question}
              </p>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <button
                  onClick={() => handleChoice("a")}
                  className="rounded-xl border-2 border-[#cfcfcf] bg-white p-4 text-center text-[16px] font-semibold text-black shadow-[2px_3px_0px_0px_#cfcfcf] transition-all hover:scale-[1.02] hover:border-[#f29337] hover:shadow-[2px_3px_0px_0px_#f29337] md:text-[18px]"
  ...
  return (
    <div className="relative size-7 overflow-hidden md:size-8">
      <div className="absolute inset-[16.67%_45.83%_16.67%_16.67%]">
        <img src="/figma/category/arrow-1.svg" alt="" className="size-full" style={{ filter: "grayscale(100%) brightness(0.6)" }} />
      </div>
      <div className="absolute inset-[45.83%_16.67%]">
        <img src="/figma/category/arrow-2.svg" alt="" className="size-full" style={{ filter: "grayscale(100%) brightness(0.6)" }} />
      </div>
    </div>
  );
}
...
function ArrowRightSmall() {
  return (
    <div className="relative h-7 w-6 overflow-hidden md:h-8 md:w-7">
      <div className="absolute inset-[45.83%_16.67%]">
        <img src="/figma/storybook/arrow-right-2.svg" alt="" className="size-full" style={{ filter: "grayscale(100%) brightness(0.6)" }} />
      </div>
      <div className="absolute inset-[16.67%_16.67%_16.67%_45.83%]">
        <img src="/figma/storybook/arrow-right-1.svg" alt="" className="size-full" style={{ filter: "grayscale(100%) brightness(0.6)" }} />
      </div>
    </div>
  );
}
...
function ArrowRightLarge() {
  return (
    <div className="relative h-9 w-8 overflow-hidden md:h-10 md:w-9">
      <div className="absolute inset-[45.83%_16.67%]">
        <img src="/figma/storybook/arrow-right-2.svg" alt="" className="size-full" />
      </div>
      <div className="absolute inset-[16.67%_16.67%_16.67%_45.83%]">
        <img src="/figma/storybook/arrow-right-1.svg" alt="" className="size-full" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative size-7 overflow-hidden md:size-9">
      <div className="absolute inset-[8.33%_16.67%_16.67%_8.33%]">
        <img src="/figma/storybook/search-1.svg" alt="" className="size-full" />
      </div>
      <div className="absolute inset-[65.42%_8.33%_8.33%_65.42%]">
        <img src="/figma/storybook/search-2.svg" alt="" className="size-full" />
      </div>
    </div>
  );
}
...
function PauseIcon() {
  return (
    <div className="relative size-8 overflow-hidden md:size-10">
      <div className="absolute inset-[12.5%_54.17%_12.5%_20.83%]">
        <img src="/figma/storybook/pause-bar.svg" alt="" className="size-full" />
      </div>
      <div className="absolute inset-[12.5%_20.83%_12.5%_54.17%]">
        <img src="/figma/storybook/pause-bar.svg" alt="" className="size-full" />
      </div>
    </div>
  );
...
function PlayIcon() {
  return (
    ...
    >
      <path
        d="M9 6.5C9 5.67157 9.92143 5.18743 10.6055 5.65149L24.6055 15.1515C25.215 15.5651 25.215 16.4349 24.6055 16.8485L10.6055 26.3485C9.92143 26.8126 9 26.3284 9 25.5V6.5Z"
        fill="white"
      />
    </svg>
  );
