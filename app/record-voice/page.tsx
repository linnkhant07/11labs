...
export default function RecordVoice() {
  ...
  const { setNarrator, setCustomVoice } = useNarrator();
  ...
  function handleRecordClick() {
    if (cloning) return;
    if (recording) void stopRecordingAndClone();
        ...
        <section className="flex w-full flex-col gap-10 rounded-[24px] bg-white p-6 shadow-[2px_2px_5px_rgba(0,0,0,0.1)] md:gap-12 md:p-10">
          ...
          <div className="flex flex-col gap-8">
            ...
            <button
              ...
              disabled={cloning}
              className="flex items-center justify-center gap-6 self-start rounded-full bg-[#f09237] px-10 py-[22px] shadow-[2px_8px_0px_0px_#db6c00,2px_2px_20px_0px_rgba(0,0,0,0.1)] transition-transform hover:scale-[1.03] active:translate-y-[2px] active:shadow-[2px_4px_0px_0px_#db6c00,2px_2px_20px_0px_rgba(0,0,0,0.1)] disabled:cursor-not-allowed disabled:opacity-70"
            >
              <MicIcon />
              <span className="font-grandstander text-[26px] font-medium text-white">
                {cloning ? "Cloning..." : recording ? "Stop & Clone" : "Record"}
              </span>
            </button>
            {error && (
              <p className="text-[16px] text-red-600" style={{ fontFamily: "var(--font-nunito), sans-serif" }}>
                {error}
              </p>
            )}
            <p
