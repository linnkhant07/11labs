...
function LibraryCard({ item, disabled, onSelect }: { item: LibraryItem; disabled?: boolean; onSelect: () => void }) {
  return (
    <button
      onClick={onSelect}
      disabled={disabled}
      className="group flex aspect-[595/842] w-full flex-col items-center justify-center gap-3 overflow-hidden rounded-[16px] bg-[#fef9f3] px-3 py-2 shadow-[2px_2px_20px_rgba(0,0,0,0.1)] transition-transform hover:scale-[1.02] focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
    >
      {item.isCreate ? (
